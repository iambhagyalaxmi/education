...
// 3. Add handleAddSubject and handleDeleteSubject
const handlersCode = `
...
// 4. Add renderSubjectsManagement
const renderUI = `
    ...
    <div className="space-y-6 animate-fade-in-up pb-10">
      <div className="flex justify-between items-center bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 mt-1">Manage subjects and curriculum for all courses</p>
        </div>
        <button onClick={() => setShowAddSubject(true)} className="flex items-center gap-2 px-4 py-2 bg-slate-900 dark:bg-indigo-600 text-white rounded-lg hover:bg-slate-800 dark:hover:bg-indigo-700 transition-colors shadow-md hover:shadow-lg font-medium">
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50 dark:bg-slate-800/50 border-b border-slate-100 dark:border-slate-800">
                ...
                <th className="p-4 font-semibold text-slate-600 dark:text-slate-300">Semester</th>
              ...
              {subjects.map(subject => (
                <tr key={subject.id} className="border-b border-slate-50 dark:border-slate-800/50 hover:bg-slate-50 dark:hover:bg-slate-800/20 transition-colors">
                  ...
                  <td className="p-4 pr-6 text-right">
                    <button onClick={() => handleDeleteSubject(subject.id)} className="p-2 text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-900/20 rounded-lg transition-colors" title="Delete Subject">
                      <Trash2 size={18} />
                    </button>
                  </td>
                </tr>
...
// 5. Add Modal HTML and switch routing
const modalHTML = `
      {/* Add Subject Modal */}
      {showAddSubject && (
